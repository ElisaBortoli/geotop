...
void time_loop(AllData *A, mio::IOManager &iomanager)
{
    ...
    S = new Statevar3D();
    allocate_and_initialize_statevar_3D(S, geotop::input::gDoubleNoValue, A->P->max_snow_layers,
    ...
    do
    {
        ...
        do
        {
            ...
            do
            {
                ...
                std::vector<mio::MeteoData> vec_meteo;  /* Defined here because it is passed to EnergyBalance
  even when USE_INTERNAL_METEODISTR is defined 13/06/2014 GG*/
...
#ifdef VERBOSE
                    lg->logsf(geotop::logger::NOTICE, "TIME-LOOP:Dtdefault:%f, JD0:%f,JDb:%f,JDe:%f", Dt, JD0, JDb, JDe);
#endif
                    tend = clock();
