...

{
    ...
    std::shared_ptr<geotop::input::ConfigStore> lConfigStore = geotop::input::ConfigStoreSingletonFactory::getInstance();
    ...
    lg->log("Start Reading METEO data files..."); // (1)
    ...
    if (success == 0)
    {
        lg->logf(" File for keyword %s is not assigned", // (2)
    ...
    std::cerr << "USE_INTERNAL_METEODISTR is defined (1)" << std::endl;
    ...
    lg->logf("Numbers of Meteo Stations read: %d", num_met_stat); // (3)
    ...
    for (size_t ii = 1; ii <= (size_t)num_met_stat; ii++)
    {
        ...
        lg->logf("Reading horizon for : %d meteo station", ii); // (4)
        //  read horizon
        met->horizon[ii - 1] =
...
#ifdef USE_INTERNAL_METEODISTR
        std::cerr << "USE_INTERNAL_METEODISTR is defined (2)" << std::endl;
        // initialize
            ...
            geotop::input::gStringNoValue)
        {
            ...
            // (b) fixing dates: converting times in the same standard time set for the
            ...
            // (d) calculate Wx and Wy if Wspeed and direction are given
            if (par->wind_as_xy == 1)
            {
            ...
            // (e) calculate Wspeed and direction if Wx and Wy are given
            ...
            // (i) calculate Wx and Wy if Wspeed and direction are given
            if (par->wind_as_xy != 1)
            {
                added_wind_xy = fill_wind_xy(
                        met->data[ii - 1], met->numlines[ii - 1], iWs, iWdir, iWsx, iWsy,
...
// s stands for string
    if (geotop::common::Variables::files[fLRs] != geotop::input::gStringNoValue) {
...
#ifdef USE_INTERNAL_METEODISTR
    std::cerr << "USE_INTERNAL_METEODISTR is defined (3)" << std::endl;

    // (o) Find the first station with shortwave radiation data
    met->nstsrad = 0;
    ...
    // (p) Find the first station with cloud data
    met->nstcloud = 0;
    do
    {
        met->nstcloud++;
    ...
    // (q) Find the first station with longwave radiation data
    met->nstlrad = 0;
    do
    {
        met->nstlrad++;
    ...
    for (i = 1; (size_t)i <= par->n_landuses; i++)
    {
            ...
            geotop::input::gStringNoValue)    // s stands for string
        {
            ...
            if (mio::FileUtils::fileExists(string(temp) + string(textfile)))
            {
                lg->logf(
   ...
   */
    ...
    if (par->total_channel > 1)
        enumerate_channels(cnet, land->LC, top->pixel_type, top->Z0, top->slope, geotop::input::gDoubleNoValue);
    ...
    if (!mio::FileUtils::fileExists(string(geotop::common::Variables::files[fwt0])
                                    + string(ascii_esri))) /** file for the initial water table NOT found */
    {
        for (i = 1; i <= long(par->total_pixel); i++)
        {
            r = top->rc_cont[i][1];
                    ...
                    mio::IOManager &iomanager)
{
    ...
    std::string filename_dem = mio::FileUtils::getFilename(filename_dem_path);
    std::string filename_geotop = mio::FileUtils::getFilename(geotop::common::Variables::files[fdem] + ".asc");
    ...
    std::cerr << top->Z0.getRows() << std::endl;
                 ...
                 GeoMatrix<double> & /*LC*/)
{
    ...
    if (!mio::FileUtils::fileExists(string(geotop::common::Variables::files[fbed]) +
                                  string(ascii_esri)))
    {
        f = fopen(geotop::common::Variables::FailedRunFile.c_str(), "w");
        fprintf(f,
...
short file_exists(short key)
{
    ...
    {
        bool is_present = mio::FileUtils::fileExists(
