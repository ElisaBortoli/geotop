...
void meteoio_read_inputmaps(TOPO *top, LAND *land, SOIL *sl, PAR *par, INIT_TOOLS *IT, mio::IOManager &iomanager)
{
    ...
    write_map(files[fdem], 0, par->format_out, top->Z0.get(), UV, number_novalue); // QUA NON ERA COMMENTATO
    ...
    //write_map(files[fdem], 0, par->format_out, top->Z0, UV, number_novalue); => PERCHÈ È COMMENTATO??!!

    std::cerr << std::endl;
    std::cerr << "(1)(a) GEOtop => Print DEM" << std::endl;
    for(std::size_t i=0; i<top->Z0->n_row; i++) {
        for (std::size_t j=0; j<top->Z0->n_col; j++) {
            std::cerr << top->Z0->co[i*top->Z0->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    ...
    for (size_t r=1; r<=top->Z0->nrh; r++)
    {
        for (size_t c=1; c<=top->Z0->nch; c++)
        {
    ...
    std::cerr << "(1)(b) MeteoIO => Print DEM" << std::endl;
    ...
    std::cerr << "(2)(a) GEOtop => Print Land Cover - BEFORE the borders check" << std::endl;
    for(std::size_t i=0; i<land->LC->n_row; i++) {
        for (std::size_t j=0; j<land->LC->n_col; j++) {
            std::cerr << land->LC->co[i*land->LC->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    ...
    std::cerr << "(2)(b) MeteoIO => Print LAND COVER (1° modo) " << std::endl;
    std::cerr << landuse.toString() << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;

    mio:: Grid2DObject landcover;
    iomanager.read2DGrid(landcover, "landcover.asc");
    std::cerr << "(2)(b) MeteoIO => Print LAND COVER (2° modo) " << std::endl;
...
//if (par->state_pixel == 1) 
    ...
    std::cerr << "(3)(a1) GEOtop => Print SKY VIEW FACTOR" << std::endl;
    for(std::size_t i=0; i<top->sky->n_row; i++) {
        for (std::size_t j=0; j<top->sky->n_col; j++) {
            std::cerr << top->sky->co[i*top->sky->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
...
//
//    std::cerr << "(3)(a2) GEOtop => Print SKY VIEW FACTOR" << std::endl;
//    for(std::size_t i=0; i<top->sky->n_row; i++) {
//        for (std::size_t j=0; j<top->sky->n_col; j++) {
//            std::cerr << top->sky->co[i*top->sky->n_col+j] << " ";
//        }
...
//    }
    ...
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    mio:: Grid2DObject sky;
    iomanager.read2DGrid(sky, "sky.asc");

    std::cerr << "(3)(b) MeteoIO => Print SKY VIEW FACTOR" << std::endl;
    std::cerr << sky.toString() << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;

    /**************************************************************************************************/
    // reading DELAY (NON C'È LA MAPPA)
//land->delay.reset(read_map(2, files[fdelay], land->LC.get(), UV, (double)number_novalue));
    ...
    std::cerr << "(4)(a1) GEOtop => Print DELAY" << std::endl;
    for(std::size_t i=0; i<land->delay->n_row; i++) {
        for (std::size_t j=0; j<land->delay->n_col; j++) {
            std::cerr << land->delay->co[i*land->delay->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    mio:: Grid2DObject delay;
   // iomanager.read2DGrid(delay, "delay.asc"); // NON ESISTE!

    std::cerr << "(4)(b) MeteoIO => Print DELAY" << std::endl;
    std::cerr << "delay.toString() NON ESISTE ma MeteoIO NON LO SA...dovrei stampare una matrice con zeri" << std::endl;
//    write_map(files[fdelay], 0, par->format_out, land->delay.get(), UV, number_novalue);
//
//    std::cerr << "(4)(a2) GEOtop => Print DELAY" << std::endl;
//    for(std::size_t i=0; i<land->delay->n_row; i++) {
//        for (std::size_t j=0; j<land->delay->n_col; j++) {
//            std::cerr << land->delay->co[i*land->delay->n_col+j] << " ";
//        }
...
//    }
    ...
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
//    /**************************************************************************************************/
    // reading SOIL MAP (soiltype.asc)
    M.reset(read_map(2, files[fsoil], land->LC.get(), UV, (double)number_novalue));
    sl->type.reset(copylong_doublematrix(M.get()));
    
    std::cerr << "(5)(1) GEOtop => Print SOILTYPE" << std::endl;
    for(std::size_t i=0; i<sl->type->n_row; i++) {
        for (std::size_t j=0; j<sl->type->n_col; j++) {
            std::cerr << sl->type->co[i*sl->type->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    mio:: Grid2DObject soiltype;
    iomanager.read2DGrid(soiltype, "soiltype.asc");

    std::cerr << "(5)(b) MeteoIO => Print SOILTYPE" << std::endl;
    std::cerr << soiltype.toString() << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;
...
//    // SLOPE
    ...
    std::cerr << "(6)(a) GEOtop => Print SLOPE" << std::endl;
    for(std::size_t i=0; i<top->slope->n_row; i++) {
        for (std::size_t j=0; j<top->slope->n_col; j++) {
            std::cerr << top->slope->co[i*top->slope->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;

    mio::Grid2DObject slope_calc(dem.cellsize, dem.llcorner, dem.slope);
    iomanager.write2DGrid(slope_calc, mio::MeteoGrids::SLOPE, mio::Date(0.));
    std::cerr << "(6)(b1) MeteoIO => Print SLOPE (calculated)" << std::endl;
    std::cerr << slope_calc.toString() << std::endl;

    mio:: Grid2DObject slope_read;
    iomanager.read2DGrid(slope_read, "slope.asc");
    std::cerr << "(6)(b2) MeteoIO => Print SLOPE (read)" << std::endl;
    std::cerr << slope_read.toString() << std::endl;
...
//    // ASPECT
    ...
    std::cerr << "(7)(a) GEOtop => Print ASPECT" << std::endl;
    for(std::size_t i=0; i<top->aspect->n_row; i++) {
        for (std::size_t j=0; j<top->aspect->n_col; j++) {
            std::cerr << top->aspect->co[i*top->aspect->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
    std::cerr << " ------------------------------------------------------------------------------------" << std::endl;

    mio:: Grid2DObject aspect;
    iomanager.read2DGrid(aspect, "aspect.asc");
    std::cerr << "(7)(b) MeteoIO => Print ASPECT" << std::endl;
    std::cerr << aspect.toString() << std::endl;
    ...
    std::cerr << "(8)(a) GEOtop => Print CURVATURE" << std::endl;
...
//
    std::cerr << "(8)(b) MeteoIO => Print CURVATURE" << std::endl;
    std::cerr << "dem.min_curvature = " << dem.min_curvature << std::endl;
    std::cerr << "dem.max_curvature = " << dem.max_curvature << std::endl;
    ...
    */
    ...
    std::cerr << "(9)(a) GEOtop => Print PIXEL_TYPE (NO IDEA of what it is)" << std::endl;
    for(std::size_t i=0; i<top->pixel_type->n_row; i++) {
        for (std::size_t j=0; j<top->pixel_type->n_col; j++) {
            std::cerr << top->pixel_type->co[i*top->pixel_type->n_col+j] << " ";
        }
        std::cerr << std::endl;
    }
