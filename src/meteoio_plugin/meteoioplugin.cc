...
// -------------------------------------------------------------------------------------------------------------------
void meteoio_writeEsriasciiMap(const string &filename, GeoMatrix<double> &gm) // (6)
{
 ...
 * raincorrfact and the snowcorrfact
 * GEOtop part_snow() method is used to
 * perform the partitions. PSUM (old HNW) and TA are read from the MeteoIO
 * MeteoData and then convert TA to celsius
 * GEOtop part_snow() method is called
 ...
 */
void merge_meteo_data(Date &current, std::vector<MeteoData> &meteo) // (9)
{
...
// -------------------------------------------------------------------------------------------------------------------
void copyGridToMatrix(Grid2DObject &gridObject, // (15)
 ...
 * @author noori
 */
void copyGridToMatrix(Grid2DObject &gridObject, GeoMatrix<double> &myGrid) // (14)
{
    for (size_t ii = 0; ii < gridObject.getNy(); ii++)
    {
        for (size_t jj = 0; jj < gridObject.getNx(); jj++)
 ...
 */
void convertToCelsius(Grid2DObject &g2d) // (19)
{
 ...
 * @param g2d The Grid2DObject that shall be changed
 */
void convertToMBar(Grid2DObject &g2d) // (20)
{
    g2d.grid2D /= 100.0;
}
// -------------------------------------------------------------------------------------------------------------------
 ...
 * @author noori
 */
void changeVWgrid(Grid2DObject &g2d, const double &vwMin) // (21)
{
 ...
 * @author noori
 */
void changeGrid(Grid2DObject &g2d, const double &val) // (22)
{
    for (unsigned int ii = 0; ii < g2d.getNx(); ii++)
    {
        for (unsigned int jj = 0; jj < g2d.getNy(); jj++)
                  ...
                  AllData *A)
{
    ...
    for (size_t ii = 0; ii < vec_meteo.size(); ii++)
    {
        if (vec_meteo[ii](MeteoData::ISWR) != IOUtils::nodata)    // ISWR is
        {
            // measured
            if (A->M->nstcloud == 0)
                A->M->nstcloud = ii + 1;  // records the first station that has cloud info
 ...
 * calling the respective methods within MeteoIO
                                    ...
                                    GeoVector<double> &tau_cloud_vec)
{
    ...
    try
    {
        ...
        {
            meteo[i](MeteoData::TAU_CLD) = (tau_cloud_vec[i + 1] == geotop::input::gDoubleNoValue
                     ? IOUtils::nodata
                     : tau_cloud_vec[i + 1]);
 ...
 */
void meteoio_interpolate(Par *par, double matlabdate, Meteo *met, Water *wat) // (10)
{
    std::cerr << "Function meteoio_interpolate called" << std::endl;
    ...
    try
    {
      ...
      if (FileUtils::fileExists(cfgfile_datassim))
        {
 ...
 * respective methods within MeteoIO
 * 1) Data is copied from MeteoIO objects to GEOtop structs
 * 2) Interpolation takes place
 * 3) Gridded data copied back to GEOtop DOUBLEMATRIX
 * 4) TA, P and RH values need to be converted as well as nodata values
...
void meteoio_interpolate_pointwise(Par *par, // (11)
                                   ...
                                   Water *wat)
{
    std::cerr << "Function meteoio_interpolate_pointwise called" << std::endl;
